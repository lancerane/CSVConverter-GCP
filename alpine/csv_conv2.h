...
// Get the absolute path to the dir in which we're running
std::string getexepath()
{
  char result[ PATH_MAX ];
  ssize_t count = readlink( "/proc/self/exe", result, PATH_MAX );

  string s = std::string( result, (count > 0) ? count : 0 ); // this includes the .exe

  // To get the last '/', reverse the string
  string reversed(s.rbegin(), s.rend()); 
  int pos = reversed.find("/");
  
  // Then remove the bit corresponsing to '...exe'
  string path = s.erase(s.length()-pos, pos);
